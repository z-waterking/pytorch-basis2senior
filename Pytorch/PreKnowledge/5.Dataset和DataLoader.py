...
for epoch in range(2):
    for i, data in enumerate(train_loader):
        ...
        print(epoch, i, "inputs", inputs.data.size(), "labels", labels.data.size())# 接下来，我们来继承 Dataset类 ，写一个将数据处理成DataLoader的类。
#
...
class DealDataset(Dataset):
    ...
    def __len__(self):
        return self.len# 实例化这个类，然后我们就得到了Dataset类型的数据，记下来就将这个类传给DataLoader，就可以了。
